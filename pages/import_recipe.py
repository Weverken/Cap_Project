...
from src.importer.extractor import extract_recipe_from_image, extract_recipe_from_text
from src.importer.url_fetcher import fetch_recipe_page_text
...
def start_new_extraction(recipe: dict) -> None:
    ...
    st.session_state["extracted_recipe"] = recipe
    ...
    if st.button("🔍 Extract Recipe", type="primary"):
        if not url:
            ...
            with st.spinner("Reading the page..."):
                fetch_result = fetch_recipe_page_text(url)

            if not fetch_result["success"]:
                st.error(f"Couldn't read that page: {fetch_result['error']}")
            else:
                with st.spinner("Extracting the recipe..."):
                    extract_result = extract_recipe_from_text(fetch_result["text"])

                if not extract_result["success"]:
                    st.error(f"Couldn't extract a recipe: {extract_result['error']}")
                else:
                    start_new_extraction(extract_result["recipe"])
